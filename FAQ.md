...
where an engineer compared different terminals. That’s where I noticed `Kitty`
and its ability to switch themes directly from the command line. This inspired
me, and I created this issue
...
external tool using `Xfconf`, which supports changing properties at runtime.
So I decided to create my own project and implement this idea as a standalone
application.
...
I also like using `Tmux`: on one pane I run `xfce4-terminal-theme-selector`, and
on another I open files like `~/.bashrc` to adjust or test things while
switching themes.
...
I took the `xfce4-terminal` source code and started reading through it.
Along the way, I found several files that turned out to be useful - they are
responsible for storing and processing the terminal’s properties:
```
...
`terminal_preferences_load_rc_file` - loads the old `xfce4-terminal`
configuration file, reads all properties, transfers them into the properties
object, and migrates the old color palette format if necessary.
